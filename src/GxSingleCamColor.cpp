...
int main()
{
    ...
    emStatus = GXGetInt(g_hDevice,GX_INT_WIDTH,&g_nwidth);
	if(emStatus != GX_STATUS_SUCCESS)
	{
        GetErrorString(emStatus);
        GXCloseLib();
	...
	emStatus = GXGetInt(g_hDevice,GX_INT_HEIGHT,&g_nheight);
	if(emStatus != GX_STATUS_SUCCESS)
	{
        GetErrorString(emStatus);
        GXCloseLib();
...
void *ProcGetImage(void* pParam)
{
    ...
    Mat m_frame;
    int frameRate = 30;
    m_frame = Mat(g_nheight,g_nwidth,CV_8UC3);
	writer.open("./1.avi",CV_FOURCC('H', '2', '6', '4'),frameRate, Size(m_frame.cols,m_frame.rows),1);
    ...
    while(g_bAcquisitionFlag)
    {
        ...
        else
        {
            ...
            memcpy(m_frame.data,g_pRGBImageBuf,pFrameBuffer->nWidth*pFrameBuffer->nHeight*3);
